<template>
  <div class="PositionManagement">
    <div class="bread-contain">
      ...
      <div class="bread-contain-right" v-if="positionManagementEditShow">
    ...
    <div v-if="indexShow">
      ...
      <div class="kl-table">
        <el-table v-if="tableShow" :max-height="maxHeight" stripe border :data="tableData.list" class="PositionManagement-table">
          ...
          <el-table-column prop="code" min-width="100" label="岗位编号">
          </el-table-column>

          <el-table-column prop="name" min-width="100" label="岗位名称">
          </el-table-column>
          ...
          <el-table-column class="cell" label="操作" fixed="right" width="200">
            <template slot-scope="scope">
              ...
              <span>|</span>
              <el-button type="text" @click="ownedUser(scope.row)">所属用户</el-button>
            </template>
          </el-table-column>
    ...
    <!-- 首页添加/修改 -->
    <PositionManagementEdit ref="PositionManagementEdit" :positionManagementEditNeedData='positionManagementEditNeedData' v-else-if="positionManagementEditShow"></PositionManagementEdit>
    <!-- 所属用户页面 -->
    <ownedUser :ownedUserNeedData='ownedUserNeedData' v-else-if="ownedUserShow" ref="ownedUser"></ownedUser>
  </div>
</template>
<script>
import PositionManagementEdit from './PositionManagementEdit'
...
export default {
  ...
  data() {
    return {
      ...
      indexShow: true,
      // 显示首页添加/修改
      positionManagementEditShow: false,
      // 显示所属用户页面
      ownedUserShow: false,
      // 显示所属用户添加/修改页面
      ...
      // 添加/修改页面所需数据
      positionManagementEditNeedData: {},
      // 所属用户页面所需数据
      ownedUserNeedData: {},
  ...
  methods: {
    ...
    init() {
      let _this = this
      let params = {
        busicode: "/post/queryForList.htm",
    ...
    // 首页添加按钮
    add(){
      this.positionManagementEditShow = true
      this.indexShow = false
      this.positionManagementEditNeedData.saveType = 'add'
    },
    // 添加/修改保存按钮（调用子页面保存方法）
    save(){
    ...
    edit(row){
      this.positionManagementEditShow = true
      this.indexShow = false
      this.positionManagementEditNeedData = row
      this.positionManagementEditNeedData.saveType = 'edit'
    },
    // 首页删除按钮
    remove(row) {
      let _this = this
      this.$confirm('是否确定删除该条数据?', '提示', {
        ...
        let params = {
          busicode: "/post/delete.htm?id=" + row.id,
        };
        this.$api.fetch({
          params: params,
          method: 'get'
    ...
    // 首页所属用户按钮
    ownedUser(row){
      this.ownedUserShow = true
      this.indexShow = false
      this.ownedUserNeedData = row
      this.crumbsData.titleList.push({ title: "所属用户" })
    },
    // 返回首页按钮
    closeDialog() {
      this.indexShow = true
      this.positionManagementEditShow = false
